...
abstract class FansaUchiwaDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE `fansa_uchiwa_data` ADD COLUMN `overallBorderColorValue` INTEGER NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE `fansa_uchiwa_data` ADD COLUMN `overallBorderWidth` REAL NOT NULL DEFAULT 0.0"
                )
            }
        }
